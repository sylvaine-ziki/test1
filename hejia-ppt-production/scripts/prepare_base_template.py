#!/usr/bin/env python3
"""Prepare the Hejia base template by making the source bullet red."""
...
def update_layout(xml_bytes):
    ...
    for shape in root.findall(".//p:sp", NS):
        ...
        for level in shape.findall(".//a:lvl1pPr", NS):
            bullet_color = level.find("a:buClr", NS)
            if bullet_color is None:
                bullet_color = ET.Element(f"{{{A_NS}}}buClr")
                level.insert(0, bullet_color)
            for child in list(bullet_color):
                bullet_color.remove(child)
            ET.SubElement(bullet_color, f"{{{A_NS}}}srgbClr", {"val": "AD0B29"})
            changed = True
    if not changed:
