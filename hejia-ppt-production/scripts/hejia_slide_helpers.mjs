...
export function addText(slide, text, x, y, w, h, {
  ...
  box.text.typeface = bold ? HEJIA.fonts.bold : HEJIA.fonts.regular;
...
export function addStoryline(slide, {
  ...
  const primaryBox = addText(
    slide,
    [primaryLine1, primaryLine2].filter(Boolean).join("\n"),
    z.x,
    20.79,
    z.w - 7.5,
    112,
...
export function addSource(slide, sourceText) {
  ...
  // The prepared master already supplies the red source bullet; do not draw another triangle.
  const sourceBox = addText(slide, `数据来源：${sourceText}`, z.x, z.y, z.w, z.h, {
  ...
  sourceBox.text.insets = { left: 14, right: 0, top: 0, bottom: 0 };
  return sourceBox;
}
